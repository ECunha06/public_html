...
if ($_SERVER['REQUEST_METHOD'] == "POST") {
    ...
    if (!($categorias and $nomeequip and $descricao and $quantidade)) {
    ...
    if ($response["message"] != "") {
        ...
        if (move_uploaded_file($_FILES['ficheiro']['tmp_name'], $_UP['pasta'] . $nome_final)) {
            ...
            $query = "INSERT INTO equipamentos (nome_equipamento, descricao, total, quantidade, imagem) VALUES (?, ?, ?, ?, ?);";
            $stmt = mysqli_prepare($conn, $query);
            $stmt->bind_param("ssiis", $nomeequip, $descricao, $quantidade, $quantidade, $nome_final);
            $stmt->execute();
            $stmt->close();
            ...
            // You might also want to log the error or redirect the user with an error message.
        }
    }
}
        ...
        <div class="w3-container w3-medium w3-card-4 w3-light-grey">
            ...
            <form class="w3-container w3-light-grey" method="post" enctype="multipart/form-data">
                <div class="card-body">
                        ...
                        ?>
                        <!--<input type="checkbox" id="categorias_informatica" name="categorias[]" class="w3-check" value="1">
                        <label for ="categorias_informatica"> Informática </label>
                        <input type="checkbox" id="categorias_eletronica" name="categorias[]" class="w3-check" value="2">
                        <label for ="categorias_eletronica"> Eletrónica </label>
                        <input type="checkbox" id="categorias_mecatronica" name="categorias[]" class="w3-check" value="3">
                        <label for ="categorias_mecatronica"> Mecatrónica </label>
                        <input type="checkbox" id="categorias_outros" name="categorias[]" class="w3-check" value="4">
                        <label for ="categorias_outros"> Outros </label>-->
                    
                    
                       <!-- <label><b>Código do Equipamento</b></label>
                        <input class="w3-input w3-animate-input" type="text" name="Cod_Equipamento" required>
                    -->
                    <p>
